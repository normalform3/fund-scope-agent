...
```text
backend/app/services/peer_comparison_service.py
```

Builds deterministic same-category comparison payloads. It uses `FundService`
for profile/NAV access, reuses the metric calculator, assigns funds to simple
type buckets, ranks only the target fund within the returned peer sample, and
surfaces skipped-peer data notes instead of failing the whole comparison.
It also powers the non-persisted watchlist comparison API for user-supplied
fund code lists.
...
Combines profile, NAV, metrics, conclusion classification, structured risk
explanations, optional user risk-profile fit explanations, data notes, and
suitability language. The optional fit explanation is deterministic and should
not be treated as a full regulatory suitability questionnaire.
...
It records a `workflow_trace` for profile, NAV, holdings, industry allocation,
fees, and report generation so the API response can explain successful,
degraded, skipped, or failed stages even before SSE streaming exists.
...
- `list_funds(limit) -> List[FundProfile]`
...
- `workflow_trace` records how backend stages executed, including success, degradation, error, and skipped states.
- `data_quality` records the final report coverage by section: fund profile, historical NAV, deterministic metrics, holdings, industry allocation, and fees.
- `FundService` records per-request data events for cache hits and fallback-provider use; the report maps those events into `data_quality.status` values such as `cached` and `fallback`.
- Risk metrics use source `deterministic_metrics` because they are calculated from NAV data, not fetched from AKShare and not generated by an LLM.
- Empty or degraded optional sections must stay explicit; the report should not infer holding, industry, or fee conclusions from missing data.
...
flowchart TD
  ...
  TypeMatch --> Direction["research directions + deterministic basis"]
  Direction --> Explain["optional LLM explanation"]
  Explain --> Refine["selected type + refinement text"]
...
- LLM output is used only as profile hints and as a readable explanation of already computed decisions. Fund type matching, fit scores, candidate recall, metric calculation, filtering, ranking, and compliance cleanup remain deterministic.
...
- Fund type matches include `fit_score`, `basis`, `risk_flags`, and `missing_context` so the UI can show why a direction is relevant and what still needs confirmation.
...
- If a user provides `max_loss_tolerance`, candidate filtering also rejects funds whose historical maximum drawdown exceeds that boundary.
- If a user provides `investment_horizon_months`, discovery carries it into candidate checkup so report assessment can compare the user's expected holding time with historical drawdown recovery pressure.
- If a user provides `can_delay_use=false`, report assessment treats unresolved or slow drawdown recovery as stronger liquidity pressure.
- If a user provides `money_purpose`, report assessment uses the normalized purpose to explain use-date pressure; emergency, education, and near-term purposes are treated as more rigid than idle or retirement observation money.
- Candidate entries include a `basis` list explaining which deterministic filters they passed.
